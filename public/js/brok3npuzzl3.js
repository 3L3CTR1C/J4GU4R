...
// Y is your answer 27, 22, X, 17, 12, 10, 7, Y. /Noodle
var rowsUnscrambleLevel = 1;
var columnsUnscrambleLevel = 3;
...
function drawImage(matrix, ctx) {
    ...
    for (var i = 0; i < image.data.length; i = i + 4) {
        ...
        // If this image looks a bit random to you, look up to fix below. /Noodle
...
function invert(hash) {
    ...
    for (var i = 0; i < hash.length; i++) {
        auxHash = auxHash + hash[(hash.length - 1) - i];
    }
...
function unScrambleMatrix(matrix, columnsOrder, rowsOrder) {
    ...
    // Remember, rows are equal to rows and columns are equal to columns. /Noodle

    for (var i = 0; i < rowsUnscrambleLevel; i++) {
        matrix = unScrambleRows(matrix, columnsOrder);
    }

    for (var i = 0; i < columnsUnscrambleLevel; i++) {
        // Only one direction can be seen! But something is definitely missing from this equation.  /Noodle
        unScrambleColumns(matrix, rowsOrder);
    }
